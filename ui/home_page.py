...
class HomePage:
    ...
    def home(self):
        ...
        helpers.HelperFunctions(self.streamlit).get_map(
        ...
        with column1:
            classes.AnalyticsClass(
                self.streamlit, original_data
            ).create_histogram_chart(x_axis="Shipment Mode")
        with column2:
            classes.AnalyticsClass(self.streamlit, map_data.iloc[:10]).create_pie_chart(
                values="TotalNumberofShipment", names="UniqueCountries"
            )
